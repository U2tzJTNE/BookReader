package com.u2tzjtne.libepub.ui.view
...
import com.u2tzjtne.libepub.model.HighlightImpl.HighlightStyle
import com.u2tzjtne.libepub.model.sqlite.HighLightTable
import com.u2tzjtne.libepub.ui.activity.ReadActivity
import com.u2tzjtne.libepub.ui.activity.ReadActivityCallback
import com.u2tzjtne.libepub.ui.fragment.DictionaryFragment
import com.u2tzjtne.libepub.ui.fragment.ReadPageFragment
import com.u2tzjtne.libepub.util.AppUtil
import com.u2tzjtne.libepub.util.HighlightUtil
...
class FolioWebView : WebView {
    ...
    private lateinit var readActivityCallback: ReadActivityCallback
    private lateinit var parentFragment: ReadPageFragment
    ...
    fun getDirection(): String {
        return readActivityCallback.direction.toString()
    }

    @JavascriptInterface
    fun getTopDistraction(unitString: String): Int {
        val unit = DisplayUnit.valueOf(unitString)
        return readActivityCallback.getTopDistraction(unit)
    }

    @JavascriptInterface
    fun getBottomDistraction(unitString: String): Int {
        val unit = DisplayUnit.valueOf(unitString)
        return readActivityCallback.getBottomDistraction(unit)
    }

    @JavascriptInterface
    fun getViewportRect(unitString: String): String {
        val unit = DisplayUnit.valueOf(unitString)
        val rect = readActivityCallback.getViewportRect(unit)
    ...
    fun toggleSystemUI() {
        uiHandler.post {
            readActivityCallback.toggleSystemUI()
        }
    }
    ...
    private fun init() {
        ...
        gestureDetector = if (readActivityCallback.direction == Config.Direction.HORIZONTAL) {
    ...
    fun setParentFragment(parentFragment: ReadPageFragment) {
        this.parentFragment = parentFragment
    }

    fun setFolioActivityCallback(readActivityCallback: ReadActivityCallback) {
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        return if (readActivityCallback.direction == Config.Direction.HORIZONTAL) {
    ...
    private fun applyThemeColorToHandles() {
        ...
        if (Build.VERSION.SDK_INT < 23) {
            val readActivityRef: WeakReference<ReadActivity> = readActivityCallback.activity
            val mWindowManagerField = ReflectionUtils.findField(ReadActivity::class.java, "mWindowManager")
            mWindowManagerField.isAccessible = true
            val mWindowManager = mWindowManagerField.get(readActivityRef.get())
            ...
            for (view in mViews) {
                ...
                val pathClassLoader =
                    PathClassLoader("/system/app/Chrome/Chrome.apk", readActivityRef.get()?.classLoader)
    ...
    private fun computeTextSelectionRect(currentSelectionRect: Rect) {
        ...
        val viewportRect = readActivityCallback.getViewportRect(DisplayUnit.PX)
